...
def run_game(qcar, world_size =[5,5], cnt_cleaner =1,):
    ...
    while len(cleaner_elements) < cnt_cleaner:
        ...
        if not is_in_elements(cleaner,cleaner_elements) and not is_in_elements(cleaner,wall_elements):
...
if __name__ == "__main__":
    world_size=[4,4]
    qcar = QAgentCar(actions =[0,0.2*np.pi,-0.2*np.pi,0.4*np.pi,-0.4*np.pi],#action the agent can perform
    ...
    run_game(qcar, world_size, RANDOM_CLEANER_CNT)
